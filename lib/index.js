...
const PackageFilter = require('@lerna/filter-packages')
...
function watch (args) {
  ...
  const targetPackages = PackageFilter(packages, args.targets, [], true, true)

  const targets = targetPackages.reduce((targets, targetPackage) => {
